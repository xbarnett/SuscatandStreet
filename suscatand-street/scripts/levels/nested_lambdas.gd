...
func _ready() -> void:
	...
	inputfAB.connectors[0].type = TypeType.new()
	inputfAB.connectors[0].value = FunctionType.new(typeA, typeB)
	...
	output1.connectors[0].type = FunctionType.new(FunctionType.new(typeA, typeB), typeC)
